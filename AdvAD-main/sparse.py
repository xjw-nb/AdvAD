...
def attack_main_advadx(model_name=None):
    ...
    def AMG_grad_func_DGI(x, t, y, eps, attack_type=None, momentum=0.9, grad_momentum=None):
        ...
        with th.enable_grad():
            ...
            #     # untarget
            #     logits = logits[choice]  # 筛选出标签预测正确的样本
            #     y = y[choice]  # 筛选出预测正确样本的标签
            #     probs = F.softmax(logits, dim=-1) # 求出其概率值
            #     probs_selected = probs[range(len(logits)), y.view(-1)]  # 取出所有预测正确的概率值
            #     zero_nums = (probs_selected == 1) * 1e-6
            #     log_one_minus_probs_selected = th.log(1 - probs_selected + zero_nums)  # 求出预测错误的样本概率值的对数（对应公式8的对数导数）
            #     grad = th.autograd.grad(log_one_minus_probs_selected.sum(), xt)[0]  # 对上一步进行求导
            # else:
            #     assert False
            grads = []
    ...
    for batch_i, (inputs_all, label_ori_all, label_tar_all) in enumerate(data_loader):
        ...
        for i in range(input.shape[0]):
            ...
            if "vim" in model_name:
                ...
                # 现在 new_mask 只包含选中块中前50%像素点的值，其他值为零
