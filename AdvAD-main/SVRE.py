...
def attack_main(model_name=None):
    ...
    def AMG_grad_func(x, t, y, eps, attack_type=None):
        ...
        with th.enable_grad():
            ...
            #     try:
            #         model_example = attacked_models_list[2]  # 选择一个模型
            #         dummy_input = x_start.clone().detach().requires_grad_(False)  # 作为输入
            #         writer.add_graph(model_example, dummy_input)  # 记录完整的计算图
            #         writer.flush()
            #         # print("成功记录计算图！")
